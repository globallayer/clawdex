"""
Route handlers for the Clawdex REST API.
...
async def search_solutions(request: SolutionSearchRequest) -> SolutionSearchResponse:
    ...
    if not all_results:
        return SolutionSearchResponse(
            found=False,
            message="No matching solutions found in Clawdex",
            solutions=[],
            source=source,
...
async def log_decision(request: DecisionLogRequest) -> DecisionLogResponse:
    """
    Log an architectural decision to Clawdex.
...
async def log_pattern(request: PatternLogRequest) -> PatternLogResponse:
    """
    Log a reusable pattern to Clawdex.
